...
namespace BXml.DocTypes.Ebml {
    ...
    public abstract class EbmlBlockDataSource : BXmlDataBlockSource {
        private static long ReadVInt(ReadOnlySpan<byte> data, int maxLength, out int vintLength) {
            ...
            if(bytesToRead == -1) return ~BXmlElementHeader.InvalidLengthDescriptorError;
        ...
        public override ReadOnlySpan<byte> ReadIdentifier(ref BXmlElementHeader header) {
            ...
            ReadOnlySpan<byte> data = GetDataBlock(4 + 8);

            long headerPos = Position;
            int identLength = VIntConsts.VIntLength[data[0]];
            ReadOnlySpan<byte> encodedIdent = data[..identLength];
            ...
            long dataLength = ReadVInt(data[identLength..], 8, out int dataVIntLength);
            ...
            long dataPos = headerPos + identLength + dataVIntLength;
