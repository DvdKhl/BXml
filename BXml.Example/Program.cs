...
using BXml.DocTypes.Matroska;
using BXml.DocTypes.MP4;
using BXml.DocTypes.MP4.Boxes;
using System;
...
namespace BXmlLibExample {
    ...
    public class EbmlBlockDataSourceExample(byte[] data) : EbmlBlockDataSource {
        private readonly byte[] data = data;
        private long offset;

        public override long Position {
            get => offset;
            set => Advance((int)checked(value - offset));
        }
        ...
        protected override ReadOnlySpan<byte> GetDataBlock(int minLength) => new ReadOnlySpan<byte>(data)[(int)offset..];
    }


    public class MP4BlockDataSourceExample(byte[] data) : MP4BlockDataSource {
        private readonly byte[] data = data;
        private int offset;
        ...
        protected override ReadOnlySpan<byte> GetDataBlock(int minLength) => new ReadOnlySpan<byte>(data)[offset..];
    }
    ...
    public class Program {
        private static void Main() {
            ...
            var b = new byte[2 * 1024 * 1024 * 1024L - 1000];
            var stream = File.OpenRead(path);
            stream.Read(b);

            var ebmlDataSource = new EbmlBlockDataSourceExample(b);
            ...
            void mkvtraverse(int depth) {
                while(reader.Next()) {
                    Console.WriteLine("".PadLeft(depth) + $"{reader.DocElement.Name}({reader.DocElement.IdentifierString}) Pos({reader.Header.Offset}) Len({reader.Header.DataLength})");

                    if(reader.DocElement == MatroskaDocType.SimpleBlock) {
                        reader.RetrieveRawValue();
                    }
        ...
        public class MP4Node {
            private static readonly HashSet<MP4DocElement> DocElementsWithData = [
            ...
            public IReadOnlyList<MP4Node> Children => children; private readonly List<MP4Node> children = [];
        ...
        public class MP4Provider {
            ...
            private static void PopulateTrack(MP4Node track) {
            ...
            private static void PopulateVideoTrack(MP4Node trackNode, in TrackHeaderBox _1, in MediaHeaderBox _2, in SampleDescriptionBox sampleDescriptionBox) {
